...
Retention prunes a closed row out of the live backlog into the archive, so a recorded captain answer routinely outlives the row's presence there.
`complete` and `verify` therefore resolve an inventory entry from the live backlog first and from that archive second, so answering a call and letting retention archive it does not wedge every later completion for that origin.
The archive is resolved from the same sources tasks-axi reads, in its order: `[markdown] archive` in the root `.tasks.toml`, then in `$HOME/.tasks-axi/config.toml`, and otherwise tasks-axi's own default of `done-archive.md` beside the addressed backlog file, so a home that keeps no root config is covered on the same terms.
The archived row is read with tasks-axi itself, through a private scratch copy whose dated `## Archived <date>` headings are renamed to the `## Done` heading its markdown parser reads, and the archive is never written.
The archive is append-only and tasks-axi does not consult it when minting an id, so a key re-raised after its earlier row was pruned can leave several archived rows under one id; the newest is authoritative, and the scratch copy keeps only that final row block, so an older recorded answer never vouches for a later unanswered close and an older unanswered close never refuses a later recorded answer.
Only the lookup widens: an archived row is closed by definition, so it satisfies the gate solely on a recorded captain answer, and an entry naming no row in either place still refuses.
The mutating subcommands keep resolving from the live backlog alone, because tasks-axi cannot address an archived row.
...
The archived-answer family proves the retention boundary with tasks-axi's own retention driving the fixture: an answered call satisfies `complete` and `verify` both before and after `--keep 0` prunes it into the configured archive, re-attesting with `--none` keeps the archived key in `decision_keys=`, an archived call closed by a bare `tasks-axi done` with no recorded answer is still refused by name, a key naming no row in either place is still refused, and a union of an archived key with a newly held live key resolves while the live call stays held, unlaundered, and open on Bearings' Captain's Call.
The same family proves it in a home whose root carries no `.tasks.toml`, both where retention prunes into tasks-axi's default archive and where only the user's tasks-axi config names the archive: the answered call satisfies `verify` and re-attestation there, and an unresolvable key is still refused by name.
It also proves the reused-key rule in both directions, again through real retention leaving two archived rows under one id: a key answered and archived, then re-raised, closed by a bare `tasks-axi done`, and archived is refused by `verify` and `complete`, while a key first closed bare and archived, then re-held, answered, and archived satisfies both.
