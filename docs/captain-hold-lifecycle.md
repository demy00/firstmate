...
The archive is the root `.tasks.toml`'s `[markdown] archive` when it configures one, and otherwise tasks-axi's own default of `done-archive.md` beside the addressed backlog file, so a home that keeps no root config is covered on the same terms.
...
The same family proves it in a home whose root carries no `.tasks.toml`, where retention prunes into tasks-axi's default archive instead: the answered call satisfies `verify` and re-attestation there, and an unresolvable key is still refused by name.
