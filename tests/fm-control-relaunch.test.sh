...
test_promoted_scout_relaunch_receives_the_current_delivery_contract() {
  ...
  for mode in no-mistakes direct-PR local-only; do
    ...
    case "$mode" in
      direct-PR)
        rule="1. Never push to the default branch (push only your \`feature/$id\` branch). Never merge a PR." ;;
      local-only)
        rule="1. Never push to any remote and never open a PR. Work only on your \`feature/$id\` branch; firstmate handles the merge into local \`main\`." ;;
      *)
    ...
    assert_grep "git checkout -b feature/$id" "$launch" \
